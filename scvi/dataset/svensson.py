...
class AnnDatasetRNA(GeneExpressionDataset):
    ...
    def __init__(self, data, n_genes=100):
        ...
        super(AnnDatasetRNA, self).__init__(
            *GeneExpressionDataset.get_attributes_from_matrix(expression_mat),
            gene_names=gene_names
        )
...
class AnnDatasetMixed(GeneExpressionDataset):
    ...
    def __init__(self, data, matching_func='l2', n_matches=3):

        assert matching_func in ['l2','l2_sort','means','cosine','cosine_sort','random']
        ...
        # Select ERCC genes
        ercc_idx_and_gene_names = [(idx, gene_name) for idx, gene_name in enumerate(list(anndataset.var.index))
        ...
        ercc_gene_indices = np.array([idx for idx, gene_name in enumerate(gene_names)
                                      if 'ercc' in gene_name.lower()])
        ...
        # Select induced zero cells and keep only genes present in at least 21% of cells
        select_cells = expression_mat.sum(axis=1) > 0
        expression_mat = expression_mat[select_cells, :]
        ...
        print("Final dataset shape :", expression_mat.shape)
        print("ERCC genes :", len([gene_name for gene_name in gene_names if 'ercc' in gene_name.lower()]))

        super(AnnDatasetMixed, self).__init__(
            *GeneExpressionDataset.get_attributes_from_matrix(expression_mat),
            gene_names=gene_names
        )
    ...
    def _matching_func(self, ref_col, mat):
        ...
        if self.matching_func == 'l2':
            ...
            return np.linalg.norm(np.sort(mat,axis=0) - np.sort(ref_col,axis=0), axis=0)
        elif self.matching_func == 'means':
            return np.abs(np.mean(mat,axis=0) - np.mean(ref_col))
        elif self.matching_func == 'cosine':
            return 1. - np.sum(mat * ref_col, axis=0) / (np.linalg.norm(mat,axis=0) * np.linalg.norm(ref_col))
        elif self.matching_func == 'cosine_sort':
            return 1.\
                   - np.sum(np.sort(mat,axis=0) * np.sort(ref_col,axis=0), axis=0) \
                   / (np.linalg.norm(mat,axis=0) * np.linalg.norm(ref_col))
        elif self.matching_func == 'random':
            np.random.seed(0)
            return np.random.uniform(0., 100., size=(mat.shape[1],))
        else:
            raise Exception("Matching function not recognized")

    def _match_given_gene(self, expression_mat, ref_gene_index, selected_genes):

        scores = self._matching_func(expression_mat[:,ref_gene_index][:,np.newaxis], expression_mat)
        scores[selected_genes] = np.inf
        new_matches = np.arange(expression_mat.shape[1])[np.argsort(scores)[:self.n_matches]]
    ...
    def _match_genes(self, expression_mat, ref_gene_indices):
        ...
        for ref_gene_index in ref_gene_indices:
            selected_genes = self._match_given_gene(expression_mat, ref_gene_index, selected_genes)

        return selected_genes

class Sven1DatasetMixed(AnnDatasetMixed):

    def __init__(self, matching_func='l2', n_matches=3):
        current_dir = os.path.dirname(os.path.realpath(__file__))
        svens = anndata.read(os.path.join(current_dir, 'svensson_chromium_control.h5ad'))

        sven1 = svens[svens.obs.query('sample == "20311"').index]
        super(Sven1DatasetMixed, self).__init__(sven1,
...
class Sven2DatasetMixed(AnnDatasetMixed):
    ...
    def __init__(self, matching_func='l2', n_matches=3):
        ...
        sven2 = svens[svens.obs.query('sample == "20312"').index]
        super(Sven2DatasetMixed, self).__init__(sven2,
                                                matching_func=matching_func,
                                                n_matches=n_matches)
